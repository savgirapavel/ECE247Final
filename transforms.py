...
@dataclass
class SpectrogramCrop:
    """Randomly crops the frequency dimension of spectrograms and resizes back to original size.
    
    This transform simulates variations in electrode positioning and frequency response,
    adding robustness to the model's handling of sEMG data.
    
    Args:
        crop_size (int): Size of the frequency crop window
        crop_prob (float): Probability of applying a crop (default: 0.5)
        resize_to_original (bool): Whether to resize back to original dimensions (default: True)
        freq_dim (int): The frequency dimension in the spectrogram (default: -1)
    """
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        # Only apply with probability crop_prob during training
        if torch.rand(1).item() > self.crop_prob:
            return tensor
            
        # Get input shape
        original_shape = tensor.shape
        freq_size = original_shape[self.freq_dim]
        ...
        slices[self.freq_dim] = slice(start_idx, start_idx + self.crop_size)
        
        # Apply cropping
        cropped = tensor[tuple(slices)]
        
        # Return cropped tensor if no resize needed
        if not self.resize_to_original:
            return cropped
            
        # Resize back to original frequency size using interpolation
        # First need to move dimensions for interpolation
        if self.freq_dim == -1:
            freq_dim_pos = len(original_shape) - 1
        else:
            freq_dim_pos = self.freq_dim
            
        # Prepare tensor for interpolation (needs [N,C,L] format for 1D interp)
        # Move frequency dimension to the last position
        trans_dims = list(range(len(original_shape)))
        trans_dims.append(trans_dims.pop(freq_dim_pos))
        x_moved = cropped.permute(trans_dims)
        
        # Reshape to format needed for F.interpolate
        x_shape = x_moved.shape
        x_reshaped = x_moved.reshape(-1, 1, self.crop_size)
        
        # Apply interpolation
        x_interp = torch.nn.functional.interpolate(
            x_reshaped, size=freq_size, mode='linear', align_corners=False
        )
        
        # Reshape back and restore original dimension order
        x_restored = x_interp.reshape(x_shape[:-1] + (freq_size,))
        
        # Move frequency dimension back to original position
        inv_trans_dims = list(range(len(original_shape) - 1))
        inv_trans_dims.insert(freq_dim_pos, len(original_shape) - 1)
        return x_restored.permute(inv_trans_dims)
